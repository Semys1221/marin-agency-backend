# Marin Agency — Full Funnel

## Overview

Marin is a full pipeline in two phases:

1. **Phase 1 — Outreach Engine (24/7 autonome)** : scrape des leads → clean → push Instantly → interested lead
2. **Phase 2 — Frontend Engine** : interested lead → landing page → call → client
...
1. **User Account** — JSON par dossier user (niche, cibles, domaines allowed)
2. **Brain (Gemini)** — génère 10 niches, annonce sur Slack
3. **Outscraper** — scrape les leads bruts
4. **Quick Clean** — MX check + syntax + Handshake (retire les évidents invalides)
5. **Deep Clean** — DB Bounce API (payant) valide les restants
6. **Campagne check** — existe dans Instantly ?
   - Non → Sequence Creator → Campaign Setup → push
   - Oui → push directement
7. **Benchmarks** — analyse perf → Kill & Replace ou Scale
8. **Loop** — retour au scraping

### Output

Un **interested lead** (un lead qui a répondu, montré de l'intérêt).
...
1. **Landing page questionnaire** — le interested lead arrive, répond à des questions
2. **Zustand cache** — toutes les réponses mises en cache côté client
3. **Calendly booking** — le lead booke un call → user créé en DB
4. **Countdown** — le frontend affiche le compte à rebours jusqu'au call
5. **Self-discovery** — contenu optionnel pour lead avant le call
6. **Detective Agent** — scrape le site web du lead (JSON) pour collecter des infos (intelligence)
7. **Pre-call emails** — React emails via Resend, envoyés avant le call (autorité)
8. **Open rate tracking** — stocké en DB
9. **Call page** — l'agent arrive, voit le nom + les infos detective + live form
10. **Live form** — questionnaire en temps réel pendant l'appel, stocké en DB
11. **Pause support** — le call peut être mis en pause, la progression est conservée
12. **Post-call decision** :
    - **Closed Won** → client
    - **Indécis** → email sequence (nurture)
    - **Not Interested** → email sequence (final)
13. **CRM** — Client + CallSession models en DB
14. **Onboarding** — logique codée dans l'app (pas Google Docs)
15. **Client Dashboard HTML** — point d'entrée CRM
...
| AI | Gemini |
| Scraping | Outscraper |
| Email validation | Handshake + DB Bounce |
| Campaigns | Instantly |
| Notifications | Slack |
...
| Email | Resend |
| Booking | Calendly |
| Detective | Custom scraper (JSON) |
| CRM | In-app (Client + CallSession models) |
...
## Diagram — Full Funnel (Flowchart DSL)
...
Dashboard API [color:green, icon:server]
...
// ===== DASHBOARD API =====
Operations Dashboard > Dashboard API: GET /api/dashboard?tenant_id=marin
Client Dashboard > Dashboard API: GET /api/dashboard?tenant_id=X
Ecommerce Dashboard > Dashboard API: GET /api/dashboard?tenant_id=X
Dashboard API > Supabase Postgres: aggregate all tables
...
*Source: https://app.eraser.io/workspace/SG0Y12i3rX6JQ8PTTKrZ*
