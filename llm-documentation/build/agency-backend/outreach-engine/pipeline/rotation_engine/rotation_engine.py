...
from slack_notifier.campaign.notifier import announce_campaign_killed, announce_campaign_scaling
...
def _log_decisions(tenant_id: str, decisions: list[dict]):
    ...
    try:
        ...
        for d in decisions:
            ...
            reason = d.get("reason", "")
            if action == "close":
                announce_campaign_killed(tenant_id, niche, reason)
            elif "scaling" in reason.lower() or "good metrics" in reason.lower():
                announce_campaign_scaling(tenant_id, niche)
    except Exception:
        pass
