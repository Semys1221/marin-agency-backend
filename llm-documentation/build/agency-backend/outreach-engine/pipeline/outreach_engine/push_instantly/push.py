...
try:
    from slack_notifier.campaign.notifier import announce_campaign_created, announce_push_complete
    _HAS_SLACK = True
except ImportError:
    _HAS_SLACK = False
...
def push_campaign(tenant_id: str, campaign_name: str, create_if_missing: bool = False, dry_run: bool = False):
    ...
    if campaign:
        ...
        instantly_id = campaign["id"]
        if _HAS_SLACK:
            announce_campaign_created(tenant_id, campaign_name, instantly_id)
    else:
    ...
    if _HAS_SLACK and total > 0:
        announce_push_complete(tenant_id, campaign_name, total)
