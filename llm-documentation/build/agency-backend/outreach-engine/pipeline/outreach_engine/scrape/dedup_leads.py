...
def dedup(tenant_id: str, apply: bool = False) -> int:
    ...
    total += _fix_clean_leads(sb, tenant_id, apply)

    log.info("── Cleaning cold_leads ───────")
    total += _fix_cold_leads(sb, tenant_id, apply)

    if total == 0:
        log.info("\nTout est propre ✅")
    elif not apply:
        log.info("\nRerun with apply=True to fix %d issue(s)", total)
